...
APIPATH="sandflies"         # API command to call on the Sandfly Server
JQFILTER=".data[] | select(.active==true and.type==\"process\") | .name"
                            # Filter for jq in the output section
...
if [[ "$OUTPUT_JSON" == "null" ]]; then
  ...
  echo "$OUTPUT_JSON" | jq -r "$JQFILTER"
fi
