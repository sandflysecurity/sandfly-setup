...
# 5.7.0
if [ -f ../clean_scripts.sh ]; then
    mkdir -p $BACKUPFOLDER
    mv ../clean_scripts.sh $BACKUPFOLDER
fi

for x in setup_config_json.sh setup_keys.sh setup_server.sh setup_ssl.sh \
    setup_ssl_renew_cert.sh setup_ssl_signed.sh setup_ssl_signed_aio.sh; do

    if [ -f $x ]; then
        mkdir -p $BACKUPFOLDER/setup_scripts
        mv $x $BACKUPFOLDER/setup_scripts
    fi
done

if [ -d ../setup_data/templates ]; then
    mkdir -p $BACKUPFOLDER
    mv ../setup_data/templates $BACKUPFOLDER
fi
# End of 5.7.0
